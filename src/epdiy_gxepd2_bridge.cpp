...
{
  // Fast path for common known colors
  switch (color)
  {
    case GxEPD_WHITE:
      return 0xFF;
  ...
  #ifdef TYPE_8G
    {
      // TYPE_8G: Convert arbitrary RGB565 to 8-bit grayscale, quantized to 4bpp (16 levels)
      ...
      uint8_t gray = (r8 * 77 + g8 * 150 + b8 * 29) >> 8;
      return (gray >> 4) * 0x11;
    }
  #else
      return 0xFF; // Default: white for unknown colors
  #endif
  }
}
...
void EpdiyDisplay::init()
{
  if (m_initialized)
  {
    Logger::log<Logger::Level::DEBUG, Logger::Topic::DISP>("EPDiy already initialized\n");
    return;
  }
...
{
  // Native panel resolution, used by GxEPD2 bridge (EpdiyEpd2) for image writes
  epd2.WIDTH = epd_width();
  epd2.HEIGHT = epd_height();
  // Rotation-aware dimensions, used by Adafruit_GFX for drawing primitives
